...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        # ÉP FP32 cho toàn bộ block
        with torch.cuda.amp.autocast(enabled=False):
            query = query.float()
            input_flatten = input_flatten.float()
...
class Deformable(nn.Module):
    def __init__(self, out_channels, depth, num_heads):
        super().__init__()
        self.blocks = nn.Sequential(*[DeformableBlock(out_channels, num_heads) for _ in range(depth)])
