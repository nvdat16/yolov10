...
class CSWinTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dim=64, depth=[2,2,6,2], split_size = [1,2,7,7],
        ...
        # Chuẩn hoá cho các stage
        self.norm2 = norm_layer(embed_dim * 2)
        self.norm3 = norm_layer(embed_dim * 4)
        self.norm4 = norm_layer(embed_dim * 8)

        # Lateral layers (đầu ra cho YOLO)
        self.lateral = nn.ModuleList([
            nn.Conv2d(embed_dim*2, embed_dim*2, 1),
            nn.Conv2d(embed_dim*4, embed_dim*4, 1),
            nn.Conv2d(embed_dim*8, embed_dim*8, 1)
        ])

        self.out_channels = [embed_dim * 2, embed_dim * 4, embed_dim * 8]
    ...
    def forward(self, x):
        ...
        for blk in self.stage4:
            if self.use_chk:
                ...
                x = blk(x)
        feat4 = x  # lưu output stage4

        # Convert thành map 2D cho YOLO
        B = feat2.shape[0]
        H2 = W2 = int(feat2.shape[1] ** 0.5)
        H3 = W3 = int(feat3.shape[1] ** 0.5)
        H4 = W4 = int(feat4.shape[1] ** 0.5)
        ...
        # Trả về list để nối sang YOLOv10 Neck/Head
        return [p3, p4, p5]
